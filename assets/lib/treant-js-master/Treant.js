...
;( function() {
    ...
    TreeNode.prototype.buildNodeFromText = function (node) {
        // IMAGE
        if (this.image) {
            //console.log(this.text['data-accountname']);
            image = document.createElement('img');
            image.src = this.image;
            //NINO
            card = document.createElement('div');
            card.className = 'tree-card-square mdl-card mdl-shadow--2dp';

            var att = document.createAttribute("data-id");      
            att.value = this.id;                          
            card.setAttributeNode(att);

            imageInCardContainer = document.createElement('div');
            imageInCardContainer.className = 'mdl-card__title mdl-card--expand';
            imageInCardContainer.style = 'background : url("'+this.image+'")';

            titleContainer = document.createElement('h6');
            titleContainer.className = "mdl-card__title-text tree-title";
            imageInCardContainer.appendChild(titleContainer);
            var text = document.createTextNode("loading...");
            titleContainer.appendChild(text);

            buttonsInCardContainer = document.createElement('div');
            buttonsInCardContainer.className = 'mdl-card__actions mdl-card--border text-grey text-center';

            //info
            buttonInfo = document.createElement('button');
            buttonInfo.className = 'mdl-button mdl-js-button mdl-button--icon';
            iconInfo = document.createElement('i');
            iconInfo.className = 'material-icons';
            var facetext = document.createTextNode("face");
            iconInfo.appendChild(facetext);
            buttonInfo.appendChild(iconInfo);
            buttonsInCardContainer.appendChild(buttonInfo);
            //tooltips
            var att = document.createAttribute("id");      
            att.value = 'tree'+this.text['data-accountname'];                          
            buttonInfo.setAttributeNode(att);
            tooltip = document.createElement('span');
            tooltip.className = 'mdl-tooltip mdl-tooltip--top';
            var att = document.createAttribute("for");      
            att.value = 'tree'+this.text['data-accountname'];                          
            tooltip.setAttributeNode(att);
            buttonsInCardContainer.appendChild(tooltip);

            if (this.id != 0) {
                //view childs
                buttonInfo = document.createElement('a');
                buttonInfo.className = 'mdl-button mdl-js-button mdl-button--icon';
                var link = document.createAttribute("href");
                //link.value = window.location.href+'/12321123';
                buttonInfo.setAttributeNode(link);
                iconInfo = document.createElement('i');
                iconInfo.className = 'material-icons';
                var facetext = document.createTextNode("supervised_user_circle");
                iconInfo.appendChild(facetext);
                buttonInfo.appendChild(iconInfo);
                buttonsInCardContainer.appendChild(buttonInfo);
                ...
                //tooltips
                var att = document.createAttribute("id");      
                att.value = 'treeView'+this.text['data-accountname'];                          
                buttonInfo.setAttributeNode(att);
                tooltip = document.createElement('span');
                tooltip.className = 'mdl-tooltip mdl-tooltip--top';
                var att = document.createAttribute("for");      
                ...
                tooltip.setAttributeNode(att);
                buttonsInCardContainer.appendChild(tooltip);
                var facetext = document.createTextNode("View downline members");
                tooltip.appendChild(facetext);
            }
            
            if (this.children.length != 2) {
                //add childs
                buttonInfo = document.createElement('button');
                buttonInfo.className = 'mdl-button mdl-js-button mdl-button--icon assignNewMember';
                iconInfo = document.createElement('i');
                iconInfo.className = 'material-icons';
                var facetext = document.createTextNode("group_add");
                iconInfo.appendChild(facetext);
                buttonInfo.appendChild(iconInfo);
                buttonsInCardContainer.appendChild(buttonInfo);
                ...
                //tooltips +this.id
                var att = document.createAttribute("id");      
                att.value = 'treeAdd'+this.text['data-accountname'];                          
                buttonInfo.setAttributeNode(att);
                tooltip = document.createElement('span');
                tooltip.className = 'mdl-tooltip mdl-tooltip--top';
                var att = document.createAttribute("for");      
                ...
                tooltip.setAttributeNode(att);
                buttonsInCardContainer.appendChild(tooltip);
                var facetext = document.createTextNode("Assign new member");
                tooltip.appendChild(facetext);
            }
            ...
            //console.log(this);
            card.appendChild(imageInCardContainer);
            card.appendChild(buttonsInCardContainer);

            node.appendChild(card);
            //END NINO

            //node.appendChild(image); ORIGINAL CODE
        }
